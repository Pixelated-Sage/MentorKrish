  ...
  return (
    <section className="relative bg-w1 py-16 px-2 sm:px-6" id = "what-we-offer">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {offerings.map((offer, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              variants={fadeIn}
              className="group relative flex flex-col bg-w2 border border-w2 rounded-3xl shadow-md overflow-hidden hover:shadow-lg transition-all"
              style={{ minHeight: '260px' }}
            >
              {/* Top Icon Bar */}
              <div className="flex items-center justify-center bg-g1/5 py-7 sm:py-9">
                <img src={offer.icon} alt="" className="h-12 w-12 sm:h-14 sm:w-14" />
              </div>

              {/* Text Content */}
              <div className="flex flex-col flex-grow px-5 py-5 sm:px-6 sm:py-6">
                <h3 className="text-g1 font-semibold text-base sm:text-lg mb-2">
                  {offer.title}
                </h3>
                <p className="text-g2 text-xs sm:text-sm md:text-base flex-grow">{offer.desc}</p>
                {/* Accent strip */}
                <div className="mt-5 h-1 w-10 md:w-12 bg-r1 rounded-full" />
              </div>
        ...
        {/* Footer Note / Tagline */}
        <div className="mt-12 text-center max-w-4xl mx-auto px-2">
